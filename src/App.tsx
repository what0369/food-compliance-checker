...
type NewsSubmission = { title: string; url: string; date: string; region: string; source: string; note: string };
...
export default function Home() {
  ...
  const [newsSubmission, setNewsSubmission] = useState<NewsSubmission>({ title: "", url: "", date: new Date().toISOString().slice(0, 10), region: "台灣", source: "", note: "" });
  ...
  function submitNewsForReview() {
    const data = { ...newsSubmission, title: clean(newsSubmission.title), url: clean(newsSubmission.url), source: clean(newsSubmission.source), region: clean(newsSubmission.region), note: clean(newsSubmission.note) };
    if (!data.title || !data.url) { setError("新聞標題與網址為必填。"); return; }
    try { const parsed = new URL(data.url); if (!/^https?:$/.test(parsed.protocol)) throw new Error(); } catch { setError("請輸入完整的 http 或 https 新聞網址。"); return; }
    const body = [`## 新聞標題`, data.title, ``, `## 新聞網址`, data.url, ``, `## 發布日期`, data.date, ``, `## 地區／主管機關`, data.region || "未提供", ``, `## 新聞來源`, data.source || "未提供", ``, `## 補充說明`, data.note || "無", ``, `---`, `管理者確認資料及同一性後，請留言：/收錄`].join("\n");
    window.open(`${NEW_ISSUE_URL}?title=${encodeURIComponent(`新聞線索：${data.title}`)}&body=${encodeURIComponent(body)}`, "_blank", "noopener,noreferrer");
    setSubmittingNews(false); setError("");
  }
  ...
  return <main>
    ...
    {submittingNews && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setSubmittingNews(false)}><section className="review-modal news-modal" role="dialog" aria-modal="true"><button className="close" onClick={() => setSubmittingNews(false)}>×</button><p className="eyebrow">NEWS SUBMISSION</p><h2>提交新聞線索</h2><p className="modal-intro">填寫後會開啟 GitHub 待審核頁面；送出不會立即公開，管理者核准後才納入共用資料庫。</p><div className="news-form"><label>新聞標題<input value={newsSubmission.title} onChange={(e) => setNewsSubmission({ ...newsSubmission, title: e.target.value })} placeholder="例如：食安中心揭2款橄欖油含礦物油"/></label><label>新聞網址<input type="url" value={newsSubmission.url} onChange={(e) => setNewsSubmission({ ...newsSubmission, url: e.target.value })} placeholder="https://..."/></label><div><label>發布日期<input type="date" value={newsSubmission.date} onChange={(e) => setNewsSubmission({ ...newsSubmission, date: e.target.value })}/></label><label>地區／主管機關<input value={newsSubmission.region} onChange={(e) => setNewsSubmission({ ...newsSubmission, region: e.target.value })} placeholder="例如：香港／食安中心"/></label></div><label>新聞來源<input value={newsSubmission.source} onChange={(e) => setNewsSubmission({ ...newsSubmission, source: e.target.value })} placeholder="例如：香港01"/></label><label>補充說明（選填）<textarea value={newsSubmission.note} onChange={(e) => setNewsSubmission({ ...newsSubmission, note: e.target.value })} placeholder="產品、品牌、主管機關或需要注意的事項"/></label></div><button className="primary analyze" onClick={submitNewsForReview}>前往 GitHub 送出審核</button><p className="github-note">需要登入免費 GitHub 帳號。管理者在議題留言「/收錄」後，網站會自動更新。</p></section></div>}

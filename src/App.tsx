...
type UploadRow = { product: string; supplier: string; brand: string; manufacturer: string; taxId: string; adUrl: string; claimText: string };
...
function makeQuery(row: UploadRow) { return [row.brand, row.product, row.supplier, row.manufacturer].filter(Boolean).slice(0, 3).join(" "); }
function newsKey(row: UploadRow) { return row.supplier || row.manufacturer || row.brand || row.product; }
function newsMatches(row: UploadRow, items: NewsItem[]) {
  const candidates = [row.supplier, row.manufacturer].map(companyCore).filter((value) => value.length >= 3);
  const productCandidates = [row.product, row.brand].map(compact).filter((value) => value.length >= 4);
...
function officialMatches(row: UploadRow, items: OfficialItem[]) {
  const companies = [row.supplier, row.manufacturer].map(companyCore).filter((v) => v.length >= 3);
  const products = [row.product].map(compact).filter((v) => v.length >= 4);
  const brands = [row.brand].map(compact).filter((v) => v.length >= 3);
  return items.flatMap((item) => {
    ...
    const productHit = products.some((name) => itemProducts.some((candidate) => strongProductMatch(name, candidate)));
    const brandHit = brands.some((name) => itemBrands.some((candidate) => candidate === name));
...
export default function Home() {
  ...
  async function runCheck() {
    ...
    try {
      ...
      const checked = rows.map<Result>((row) => {
      ...
      setResults(checked);
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ fileName, rowCount: rows.length, results: checked, checkedAt: new Date().toISOString() }));
  ...
  return <main>
    ...
    <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow">零額外查詢費</p><h1>上傳 Excel，免費完成<br/><em>官方與新聞初查</em></h1><p className="lead">自動比對食藥署一年內公開資料、搜尋新聞線索；再開啟購物頁並貼入宣稱文字，依食品廣告法規做風險初篩。統編與商品網址皆為選填。</p><div className="source-chips"><span>食藥署開放資料</span><span>免費新聞搜尋</span><span>購物網站宣稱</span><span>瀏覽器保存進度</span></div></div>
      <div className="upload-card"><div className="step-label"><span>1</span>匯入查核清單</div><button className={`dropzone ${fileName ? "has-file" : ""}`} onClick={() => inputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const file = e.dataTransfer.files[0]; if (file) readFile(file); }}><input ref={inputRef} type="file" accept=".xlsx,.xls,.csv" hidden onChange={(e) => e.target.files?.[0] && readFile(e.target.files[0])}/><span className="file-icon">{fileName ? "✓" : "XL"}</span><strong>{fileName || "拖曳 Excel 到這裡，或點擊選擇"}</strong><small>{fileName ? `已讀取 ${rows.length} 筆有效資料` : "產品、品牌或業者至少一項｜統編與網址選填"}</small></button>{error && <p className="error">{error}</p>}{sourceWarning && <p className="warning-box">{sourceWarning}</p>}<div className="upload-actions"><button className="text-btn" onClick={downloadTemplate}>↓ 下載新版範本</button><button className="text-btn" onClick={useSample}>使用範例資料</button></div><button className="primary" disabled={!rows.length || loading} onClick={runCheck}>{loading ? <><span className="spinner"/>免費自動查核中…</> : `免費自動查核${rows.length ? `（${rows.length} 筆）` : ""}`}</button>{progress && <p className="progress">{progress}</p>}<p className="privacy">不使用付費 AI API；Excel 在瀏覽器解析，查核結果保存在此裝置。</p></div>
    </section>
